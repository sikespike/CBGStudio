package com.cbg.studio.client.gwt.animation;
...
public class AnimationBonesData {
    ...
    public static List<AngleAndPosition> cloneAngleAndMatrix(
            List<AngleAndPosition> matrixs) {
    ...
    public AnimationBonesData(JsArray<AnimationBone> bones,
            List<AngleAndPosition> bonesMatrix) {
    ...
    public static List<Vector3> boneToRelativePositions(
            JsArray<AnimationBone> bones) {
        List<Vector3> positions = new ArrayList<Vector3>();
        for (int i = 0; i < bones.length(); i++) {
            AnimationBone bone = bones.get(i);
    ...
    public Vector3 getMatrixPosition(int index) {
        /*
         * Matrix4 pos= bonesMatrixs.get(index).getMatrix(); return
         * GWTThreeUtils.toPositionVec(pos);
         */
        return bonesMatrixs.get(index).getPosition().clone();
    }

    public static List<AngleAndPosition> boneToAngleAndMatrix(
            JsArray<AnimationBone> bones, AnimationData animationData,
            int animationIndex) {
        ...
        for (int i = 0; i < bones.length(); i++) {
            ...
            // mx.setTranslation(motionPos.getX(), motionPos.getY(),
            // motionPos.getZ());
            // mx.setRotationFromQuaternion(GWTThreeUtils.jsArrayToQuaternion(motion.getRot()));

            Matrix4 mx = THREE.Matrix4().makeTranslation(motionPos.getX(),
                    motionPos.getY(), motionPos.getZ());
            Matrix4 mx2 = THREE.Matrix4().makeRotationFromQuaternion(
                    GWTThreeUtils.jsArrayToQuaternion(motion.getRot()));
            mx.multiply(mx2);
            /*
             ...
             * //LogUtils.log(mx2); //LogUtils.log(rot); Vector3
             * rotR=GWTThreeUtils.degreeToRagiant(rot);
             * //LogUtils.log("rot:"+ThreeLog.get(rotR));
             * //LogUtils.log("rot-r:"
             * +ThreeLog.get(GWTThreeUtils.radiantToDegree(rotR))); Matrix4
             * changed=GWTThreeUtils.rotationToMatrix4(rotR);
             * 
            ...
            /*
             * Vector3 tmpRot=THREE.Vector3(); tmpRot.setRotationFromMatrix(mx);
             * Vector3 tmpPos=THREE.Vector3(); tmpPos.setPositionFromMatrix(mx);
             */
            // LogUtils.log(tmpPos.getX()+","+tmpPos.getY()+","+tmpPos.getZ());
            // LogUtils.log(Math.toDegrees(tmpRot.))
            boneMatrix.add(new AngleAndPosition(motion.getAngle(), motionPos,
                    mx));
        }
        return boneMatrix;
    }

    public static List<Matrix4> boneToMatrix(JsArray<AnimationBone> bones,
            AnimationData animationData, int index) {
