...
public class AngleAndPosition {
    ...
    public void updateMatrix() {
        ...
        Matrix4 m2 = THREE.Matrix4().makeRotationFromEuler(
                THREE.Euler(vec.getX(), vec.getY(), vec.getZ(), "XYZ"));
        matrix = THREE
                .Matrix4()
                .makeTranslation(position.getX(), position.getY(),
                        position.getZ()).multiply(m2);
        // matrix.setTranslation(position.getX(), position.getY(),
        // position.getZ());
        // matrix.setRotationFromEuler(GWTThreeUtils.degreeToRagiant(angle),
        // "XYZ");
    }
