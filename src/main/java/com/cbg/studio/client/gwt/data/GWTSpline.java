...
public class GWTSpline {
    ...
    public static XYObject get2DPoint(List<XYObject> points, double value) {
        ...
        double x = GWTSpline.interpolate(points.get(c[0]).getX(),
                points.get(c[1]).getX(), points.get(c[2]).getX(),
                points.get(c[3]).getX(), weight);
        double y = GWTSpline.interpolate(points.get(c[0]).getY(),
                points.get(c[1]).getY(), points.get(c[2]).getY(),
                points.get(c[3]).getY(), weight);

        return XYObject.create(x, y);
    }

    // Catmull-Rom
    public static final native double interpolate(double p0, double p1,
