...
public class GeometryUtils {
    ...
    public static final native Vector3 randomPointInTriangle(Vector3 vectorA,
            Vector3 vectorB, Vector3 vectorC)/*-{
                                             return $wnd.THREE.GeometryUtils.randomPointInTriangle(vectorA, vectorB, vectorC );
                                             }-*/;

    public static final native Vector3 randomPointInFace(Face face,
            Geometry geometry, boolean useCachedAreas)/*-{
                                                      return $wnd.THREE.GeometryUtils.randomPointInFace(face,geometry,useCachedAreas);
                                                      }-*/;

    public static final native Vector3 randomPointsInGeometry(
            Geometry geometry, int n)/*-{
                                     return $wnd.THREE.GeometryUtils.randomPointsInGeometry(geometry,n);
                                     }-*/;

    public static final native double triangleArea(Vector3 vectorA,
            Vector3 vectorB, Vector3 vectorC)/*-{
