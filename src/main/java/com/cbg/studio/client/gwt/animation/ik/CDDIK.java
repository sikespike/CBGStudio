package com.cbg.studio.client.gwt.animation.ik;
...
public class CDDIK {
    ...
    public Matrix4 doStep(Vector3 lastJointPos, Vector3 jointPos,
            Matrix4 jointRot, Vector3 targetPos) {
    ...
    public Matrix4 getStepAngleMatrix(Vector3 parentAngle,
            Vector3 lastJointPos, Vector3 jointPos, Matrix4 jointRot,
            Vector3 targetPos) {
        Vector3 parentVec = parentAngle.clone().negate();

        Matrix4 mx = GWTThreeUtils.degitRotationToMatrix4(parentVec.getX(),
                parentVec.getY(), parentVec.getZ());

        /*
         * Vector3 jointPosParent=jointPos.clone();
         * mx.multiplyVector3(jointPosParent);
         * 
         * Vector3 jointVector=mx.multiplyVector3(lastJointPos.clone()).subSelf(
         * jointPosParent).normalize(); Vector3
         * targetVector=mx.multiplyVector3(targetPos
         * .clone()).subSelf(jointPosParent).normalize();
         */
    ...
    public Matrix4 getStepAngleMatrix(Vector3 lastJointPos, Vector3 jointPos,
            Matrix4 jointRot, Vector3 targetPos) {
