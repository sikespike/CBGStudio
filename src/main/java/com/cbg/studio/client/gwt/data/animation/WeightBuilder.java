package com.cbg.studio.client.gwt.data.animation;
...
import com.cbg.studio.client.gwt.animation.NameAndVector3;
...
/**
 * creating 2-bone indices and weight from geometry and bone position. some
 * algorithm is totally experimental.
 * 
 * @author aki
 * 
 */
public class WeightBuilder {
    ...
    public static Vector4 findNearSingleBone(
            List<NameAndVector3> nameAndPositions, Vector3 pos,
    ...
    public static Vector4 findNearBoneParentAndChildren(
            List<NameAndVector3> nameAndPositions, Vector3 pos,
            JsArray<AnimationBone> bones, int xloop) {
        Vector3 pt = nameAndPositions.get(0).getVector3().clone();
        Vector3 near = pt.sub(pos);
        int index1 = nameAndPositions.get(0).getIndex();
        double near1 = pt.length();
        ...
        if (indexHalf != -1) {
            if (bones.get(index1).getParent() == indexHalf
                    || bones.get(indexHalf).getParent() == index1
                    || indexHalf == index1) {
                index2 = indexHalf;
                near2 = nearHalf;
            }
        ...
        if (index1 == index2) {
            ...
            return THREE.Vector4(index1, index2, (total - near1) / total,
                    (total - near2) / total);
        }
    }

    public static Vector4 findNearBoneStartAndHalfParentAndChildren(
            List<NameAndVector3> nameAndPositions, Vector3 pos,
            JsArray<AnimationBone> bones, int xloop) {
        ...
        /*
         * int indexHalf=-1; double nearHalf=0; for(int
         * i=1;i<nameAndPositions.size();i++){
         * if(nameAndPositions.get(i).getName().startsWith(KEY_END)){ continue;
         * } Vector3 npt=nameAndPositions.get(i).getVector3().clone();
         * near=npt.sub(pos); double l=near.length(); if(l<near1){
         * indexHalf=nameAndPositions.get(i).getIndex(); nearHalf=l; }else
         * if(l<near2){ indexHalf=nameAndPositions.get(i).getIndex();
         * nearHalf=l; } }
         * 
         * 
         * 
         * 
         * if(indexHalf!=-1){ if(bones.get(index1).getParent()==indexHalf ||
         * bones.get(indexHalf).getParent()==index1 ||indexHalf==index1){
         * index2=indexHalf; near2=nearHalf; } }
         */
    ...
    public static Vector4 findNearBoneAggresive(
            List<NameAndVector3> nameAndPositions, Vector3 pos,
            JsArray<AnimationBone> bones) {
        Vector3 pt = nameAndPositions.get(0).getVector3().clone();
        Vector3 near = pt.sub(pos);
         ...
         * TODO future support
         * 
         * if(bones.get(index1).getParent()!=index2 &&
         * bones.get(index2).getParent()!=index1){ index2=index1; }
         */
    ...
    public static Vector4 findNearSpecial(
            List<NameAndVector3> nameAndPositions, Vector3 pos,
            JsArray<AnimationBone> bones, int vindex) {
        ...
        if (index1 == index2) {
            ...
            return THREE.Vector4(index1, index2, (total - near1Length) / total,
                    (total - near2Length) / total);
        }

    }

    public static void autoWeight(Geometry geometry,
            JsArray<AnimationBone> bones, List<List<Vector3>> endSites,
        ...
        for (int i = 0; i < geometry.vertices().length(); i++) {
            ...
            if (mode == MODE_NearSingleBone) {
                ret = findNearSingleBone(nameAndPositions, geometry.vertices()
                        .get(i), bones);
            } else if (mode == MODE_NearSpecial) {
                ret = findNearSpecial(nameAndPositions, geometry.vertices()
                        .get(i), bones, i);
            } else if (mode == MODE_NearAgressive) {
                ret = findNearBoneAggresive(nameAndPositions, geometry
                        .vertices().get(i), bones);
            } else if (mode == MODE_NearParentAndChildren) {
                ret = findNearBoneParentAndChildren(nameAndPositions, geometry
                        .vertices().get(i), bones, 2);
            } else if (mode == MODE_NearParentAndChildrenAgressive) {
                ret = findNearBoneParentAndChildren(nameAndPositions, geometry
                        .vertices().get(i), bones, 3);
            } else if (mode == MODE_MODE_Start_And_Half_ParentAndChildrenAgressive) {
                ret = findNearBoneStartAndHalfParentAndChildren(
                        nameAndPositions, geometry.vertices().get(i), bones, 3);
    ...
    public static boolean isConnected(Vector4 v4, JsArray<AnimationBone> bones) {
        ...
        return bones.get((int) v4.getX()).getParent() == v4.getY()
                || bones.get((int) v4.getY()).getParent() == v4.getX();
    }

    // is this slow?
    public static Set<NameAndVector3> sortNameAndPosition(
            List<NameAndVector3> nameAndPositions, Vector3 vertexPos) {

        Map<NameAndVector3, Integer> map = new HashMap<NameAndVector3, Integer>();
        final Ordering<NameAndVector3> orders = Ordering.natural().reverse()
                .nullsLast().onResultOf(Functions.forMap(map, null));

        return ImmutableSortedMap.copyOf(map, orders).keySet();
    }

    public static int findClosedBone(List<NameAndVector3> nameAndPositions,
            Vector3 vertexPos, int ignoreIndex) {
        int minIndex = -1;
    ...
    public static void autoWeight(Geometry geometry,
            JsArray<AnimationBone> bones, int mode,
            JsArray<Vector4> bodyIndices, JsArray<Vector4> bodyWeight) {
    ...
    public static List<NameAndVector3> boneToNameAndPosition(
            JsArray<AnimationBone> bones) {
        ...
        for (int i = 0; i < bones.length(); i++) {
            ...
            if (bone.getParent() != -1) {
                ...
                if (pos.getX() != 0 || pos.getY() != 0 || pos.getZ() != 0) {
                    ...
                    lists.add(new NameAndVector3(parentName, endPos,
                            parentIndex));// start pos
                    lists.add(new NameAndVector3(KEY_HALF + parentName, half,
                            parentIndex));// half pos

                    if (!hasChild.contains(i)) {
                        // end
                        // LogUtils.log("end "+bone.getName());
                        Vector3 end = pos.clone().multiplyScalar(2)
                                .add(parentPos);
                        lists.add(new NameAndVector3(KEY_END + bone.getName(),
                                end, i));// end pos
                    }
                } else {
            ...
            if (endSites != null) {
                ...
                for (Vector3 endSite : ends) {
                    lists.add(new NameAndVector3(KEY_ENDSITE + bone.getName(),
                            pos.clone().add(endSite), i));//
                }
            }
