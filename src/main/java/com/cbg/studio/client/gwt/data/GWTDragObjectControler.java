...
public class GWTDragObjectControler {
    ...
    public GWTDragObjectControler(Scene scene, Projector projector) {
        ...
        mouseCatchPlane = THREE.Mesh(THREE.PlaneGeometry(2000, 2000, 10, 10),
    ...
    public void copyIntersectedPosition() {
        ...
        mouseCatchPlane.getPosition().copy(
                GWTThreeUtils.toPositionVec(intersectedDraggablekObject
                        .getMatrixWorld()));
    ...
    public void setIntersectedDraggablekObject(
            Object3D intersectedDraggablekObject) {
    ...
    /*
     * ** sometime it error happend,check double
     * mouseMoved=(Math.abs(diffX)+Math.abs(diffY));double
     * length=newPos.clone().
     * subSelf(getCurrentIkData().getTargetPos()).length();
     * if(length<mouseMoved*5){
     */
    public Vector3 moveSelectionPosition(int mouseX, int mouseY,
            int screenWidth, int screenHeight, Camera camera) {
        if (isSelected()) {
            try {
                Ray ray = projector.gwtCreateRay(mouseX, mouseY, screenWidth,
                        screenHeight, camera);
                JsArray<Intersect> intersects = ray
                        .intersectObject(mouseCatchPlane);

                Vector3 newPos = intersects.get(0).getPoint()
                        .subSelf(draggableOffset);
                log = "newPos-raw:" + ThreeLog.get(newPos);
                Vector3 parentPos = THREE.Vector3();
                if (selectedDraggablekObject.getParent() != null) {
                    // selectedDraggablekObject.getParent().updateMatrixWorld(true);
                    // //if call it bone moving will be broken.i have no idea.
                    parentPos = GWTThreeUtils
                            .toPositionVec(selectedDraggablekObject.getParent()
                                    .getMatrixWorld());
                ...
                Vector3 rotation = GWTThreeUtils
                        .rotationToVector3((selectedDraggablekObject
                                .getParent().getMatrixWorld()));
                // rotM.getInverse(selectedDraggablekObject.getMatrixRotationWorld());

                log += "parentRot:"
                        + ThreeLog.get(GWTThreeUtils.radiantToDegree(rotation));
                 ...
                 * obj=obj.getParent(); }
                 * 
                 * LogUtils.log("parent-rot:"+ThreeLog.get(GWTThreeUtils.
                 * radiantToDegree(parentRotation)));
                 */
                 ...
                 * ; log("m:"+ThreeLog.get(GWTThreeUtils.toPositionVec(
                 * selectedDraggablekObject.getMatrix())));
                 * log("mw:"+ThreeLog.get
                 * (GWTThreeUtils.toPositionVec(selectedDraggablekObject
                 * .getMatrixWorld())));
                 */
    ...
    public void selectObject(Object3D target, int mouseX, int mouseY,
            int screenWidth, int screenHeight, Camera camera) {
        try {
            ...
            JsArray<Intersect> pintersects = ray
                    .intersectObject(mouseCatchPlane);
            draggableOffset.copy(pintersects.get(0).getPoint()).subSelf(
                    mouseCatchPlane.getPosition());
            /*
             * make a problem? if(draggableOffset.getX()<0.0001){
             * draggableOffset.setX(0); } if(draggableOffset.getY()<0.0001){
             * draggableOffset.setY(0); } if(draggableOffset.getZ()<0.0001){
