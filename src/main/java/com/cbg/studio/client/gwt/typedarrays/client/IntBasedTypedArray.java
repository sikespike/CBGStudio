...
/**
 * Abstract parent class for all {@link TypedArray}s which use int to get/set
 * the inner values. Do not rely on this class as it's not part of the
 * specification and only introduced to simplify the implementation of the Int*
 * and Uint* Arrays.
 * 
 * @param <T>
 *            the type of the {@link TypedArray} itself. Used for methods that
 *            use parameters that must have the same type than the TypedArray
 *            itself.
 */
public abstract class IntBasedTypedArray<T extends IntBasedTypedArray<T>>
     ...
     * 
     * Values that are out of the range for the type used by this TypedAray are
     * silently casted to be in range.
     * 
     * @param index
     *            the index relative to the beginning of the TypedArray.
     * @param value
     ...
     * int version ( {@link IntBasedTypedArray#set(int,int)}). Please note that
    ...
    /**
     * Writes multiple values to the TypedArray using the values of the given
     * Array. Pay attention: Avoid using long values in GWT if possible (
     * {@link "http://code.google.com/intl/de-DE/webtoolkit/doc/latest/DevGuideCodingBasicsCompatibility.html#language"}
     * ). This method has poor performance in production mode compared with the
     * int[] version ( {@link IntBasedTypedArray#set(int[])}). Please note that
     * in production mode int, short and byte are handled as 64Bit floating
     * point values, so you can use them for values >2^31-1. Keep in mind that
     * not every long value can be represented exactly by 64Bit floating values.
     * Be aware that this won't work correctly in dev mode and no literals above
    ...
    /**
     * Writes multiple values to the TypedArray using the values of the given
     * Array. Writes the values beginning at the given offset. Pay attention:
     * Avoid using long values in GWT if possible (
     * {@link "http://code.google.com/intl/de-DE/webtoolkit/doc/latest/DevGuideCodingBasicsCompatibility.html#language"}
     * ). This method has poor performance in production mode compared with the
     * int[] version ( {@link IntBasedTypedArray#set(int[], int)}). Please note
     * that in production mode int, short and byte are handled as 64Bit floating
     * point values, so you can use them for values >2^31-1. Keep in mind that
     * not every long value can be represented exactly by 64Bit floating values.
     * Be aware that this won't work correctly in dev mode and no literals above
     * that limit are supported in Java.
    ...
    /**
     * Implementation for getLong that returns the value as String to be later
     * parsed as long.
     * 
     * @param index
     *            the index relative to the beginning of the TypedArray.
    ...
    /**
     * Implementation for setting long values using Strings as longs are
     * emulated in GWT and can't be directly used in JSNI.
     * 
     * @param index
